...
private[etl] object Dim {
  ...
  private[etl] def getDimensionColsToSetKeyOrLoadingFact(dimName: String, configDwEtl: ConfigDwEtl): List[String] = {
    ...
    val colsInFactSchemasToSetKeyOrToLoadFactTables = colsInFactSchemasToSetKey.concat(colsInFactSchemasToLoadFactTables) // replaced union with concat as distinct that follows gets rid of duplicates
      .distinct
...
abstract class Dim(private val dimName: String)
  ...
  private def removeCurrentAndFutureEffectiveDates(dfExistingDim: DataFrame, effectiveDate: Date): DataFrame = {
    ...
    dfForChangedCompoundKeys
      ...
      .select(dimColsInclEtlOnes.head, dimColsInclEtlOnes.tail.toIndexedSeq: _*) // select columns in the correct order
  }
  ...
  private def createNewDimVersionForTypeOneChangesOnly(dfExistingDim: DataFrame): (DataFrame, DataFrame) = {
    ...
    val typeZeroAndThreeSelectAndJoin = for ((typeZeroOrThreeCol, scdType) <- (typeZeroCols zip List.fill(typeZeroCols.length)(0))
      .concat(typeThreeCols zip List.fill(typeThreeCols.length)(3))
      .distinct
                                             )
    ...
    val dfForTheExistingCompoundKey = sparkModelObject.sql(sqlForTheExistingCompoundKey)
      .pipe(q => if (hasTypeZeroOrThreeCols) q.select(dimColsInclEtlOnes.head, dimColsInclEtlOnes.tail.toIndexedSeq: _*) else q) // The optional call to select columns in the right order
    ...
    */
    ...
    val dfNewCompoundKey = dfWithTypeOneChanges.join(dfExistingDim.where("MostRecentIndicator = TRUE"),
      ...
      .select(dimColsInclEtlOnes.head, dimColsInclEtlOnes.tail.toIndexedSeq: _*) // "leftanti" moves the CompoundKey to the front - so select columns in correct order
  ...
  private def incorporateTypeOneIntoTypeTwo(
    ...
    val dfTypeOneColumnChangedForExistingWithSurrogKeySet = sparkModelObject.sql(sqlTypeOneColumnChangedForExistingWithSurrogKeySet)
      .select(dimColumnsInclEtlCols.head, dimColumnsInclEtlCols.tail.toIndexedSeq: _*) // select columns in the correct order

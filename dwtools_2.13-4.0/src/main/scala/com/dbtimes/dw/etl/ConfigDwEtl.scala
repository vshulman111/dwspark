...
final private[etl] class ConfigDwEtl(private[etl] val configDwEtl: Config) extends ConfigDwJobsCommon {
  ...
  private[etl] def getDimSourceNamesOfFact(factName: String): List[String] = {
    val factConfig = getFactConfiguration(factName)
    if (!factConfig.hasPath("dimSources")) {
      List.empty[String]
    }
    else {
      val factDimSources = factConfig.getConfigList("dimSources").asScala.toList
      factDimSources map {
  ...
  private[etl] def getDimensionColsUsedInLoadingAllFacts(dimName: String): List[String] = {
    ...
    val listOfColsFromThisDimensionToLoadEachFact = for (
      ...
      allDimensionColumnsUsedForLoadingFact = if (dimensionSources.exists(_ == dimName)) {
        val dimensionSourcesToLoadFact = factConfig.getConfigList("dimSources").asScala.toList
   ...
   * fields in the dimension happen to be the same as in fact schema, but they can be different
  ...
  private[etl] def getFactFilePath(factName: String, isForKeySetters: Boolean = false): String = {
    if (getIsFactsDestinationFile) {
      ...
      factFilePathWithPlaceholder.replace( "___", factName )
    }
    else {
  ...
  /**
   * For "unknownValue" null is allowed value. getSchemaColNamesAndPropertyValues will skip columns
   * where "unknownValue": null. This is good as the code handles missed unknown value as null.
   * We do require to specify "unknownValue" for natural key. That is done to make the user consciously define
   * the unknown even if it is null as it used to delete duplicate records with same natural keys.
   * @param dimName
   * @return
   */
  ...
  private[etl] def getDimensionFilePath(dimName: String): String = {
    if (getIsDimDestinationFile) {
      ...
      dimFilePathWithPlaceholder.replace( "___", dimName )
    }
    else {
   ...
   *
   * @param dimName
   * @return the name of the dimension key. There can be only one dimension key column, so return the first one in the list
   */
  private[etl] def getDimensionKeyCol(dimName: String): String =
    super.getSchemaColNamesWithFlag(getDimensionConfiguration(dimName), "schema", "isKey", true, Some(false)).head
  ...
  private[etl] def getDimTypeTwoCols(dimName: String): List[String] =
    super.getSchemaColNamesWithIntValue(getDimensionConfiguration(dimName), "schema", "scdType", 2, Some(1))

  private[etl] def getDimTypeZeroCols(dimName: String): List[String] =
    super.getSchemaColNamesWithIntValue(getDimensionConfiguration(dimName), "schema", "scdType", 0, Some(1))

  private[etl] def getDimTypeThreeCols(dimName: String): List[String] =
    super.getSchemaColNamesWithIntValue(getDimensionConfiguration(dimName), "schema", "scdType", 3, Some(1))
  ...
  private[etl] def getDimTypeOneCols(dimName: String): List[String] = {
    super.getSchemaColNames(getDimensionConfiguration(dimName), "schema")
      ...
      .diff(getDimTypeZeroCols(dimName))
      .diff(getDimTypeThreeCols(dimName))
  }
  ...
  private[etl] def getDimSourceNamesOfDim(dimName: String): List[String] = {
    val dimConfig = getDimensionConfiguration(dimName)
    if (!dimConfig.hasPath("dimSources")) {
      List.empty[String]
    }
    else {
      val dimDimSources = dimConfig.getConfigList("dimSources").asScala.toList
      dimDimSources map {
        case dimSourceName: Config => dimSourceName.getString("name")
      }
    }
  ...
  private[etl] def getDimensionColsUsedInLoadingDimension(dimName: String, dimDimSourceName: String): List[String] = {
    ...
    // get columns from only one this dimension from all dimension sources for this fact table
    val dimensionSourcesToLoadDimensionConfig = dimConfig.getConfigList("dimSources").asScala.toList

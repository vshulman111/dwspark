...
import com.dbtimes.dw.common.LogFile.{dwlogger => comparerLog}
import com.dbtimes.dw.common._
import org.apache.spark.sql.{DataFrame, SparkSession}

private[sourcecomparer] class LoaderOfParquetFileSource(private val scenario: ScenarioConfig, private val isLeftSource: Boolean) extends ScenarioSourceLoader {
  ...
  private[sourcecomparer] def loadSource( ): DataFrame = {
    ...
    comparerLog.info("** Processing PARQUET file " + sourceFilePath)
    val df = spark.read.format("org.apache.spark.sql.execution.datasources.parquet.ParquetFileFormat")
      .load(sourceFilePath)

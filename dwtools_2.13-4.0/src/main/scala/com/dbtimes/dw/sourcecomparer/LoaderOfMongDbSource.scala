...
private[sourcecomparer] class LoaderOfMongDbSource(private val scenario: ScenarioConfig, private val isLeftSource: Boolean) extends ScenarioSourceLoader {

  private[sourcecomparer] def loadSource( ): DataFrame = {
    val df = DataFrameHelper.readDfFromMongoDb(
      ...
      scenario.getSchema(isLeftSource) )

...
private[sourceloader] class ProcessorOfDBMSSource(private val action: LoadAction ) extends ActionProcessor
  ...
  private[sourceloader] def process: Unit = {
    ...
    // Do not allow incremental load configuration to run on initial loaad
    // as SQL will have syntax for Watermark Columns that will fail on initial load
    if ( action.getIsInitialLoad && action.getIsIncrementalLoadDefined ) {
      val message = "Skipped initial load for incremental load action. For initial load define separate action without incremental load attributes."
      createLoadControlRecordFromFileDestinationParquet(action, timeStart, effectiveDate = null, isSuccess = false, message )
  ...
  private def loadData(timeStart: Long): Unit = {
    ...
    try {
      val sourceTableChangesSqlAsOption = if (isDoIncrementalLoad) {
        ...
        val sourceTableChangesSql = placeholderAndMaxValues.foldLeft(action.getIncrementalLoadSourceTableChangesSql) {
          case (currentSql, (placeholder, colName, null)) =>  throw new RuntimeException(s"""Loader Configuration or data ERROR: MAX value is NULL for column $colName used for incremental load when reading previous version of the file ${action.getDestinationFilePath} """)
      ...
      val dfMerged = if (isDoIncrementalLoad) {
        val dfAllMergeKeys = if (action.getPartitionColumn.isDefined && action.getMergeKeysList.contains(action.getPartitionColumn.get)) {
          readFromDBMSSource(
            action.getUser,
            action.getPassword,
            action.getConnectionUri,
          ...
          // Do not use partition column, even if defined, because it may or may not be part of merge keys
          readFromDBMSSource(
            action.getUser,
            action.getPassword,
            action.getConnectionUri,
        ...
        mergeSourceDataChanges(action, dfNewData, isNewDataPreparedForMerge = false, dfAllMergeKeys, effectiveDate)
      }
      else {
        mergeSourceData(action, dfNewData, effectiveDate)
      }
  ...
  private[sourceloader] def buildQueryToGetAllUniqueKeys(action: LoadAction): String = {
    val query = if (action.getIsSqlServerSource || action.getIsOracleSource) {
      ...
      // If it is a select expression - take it as is. Otherwise build a query
      ...
      if ( uniqueKeysSqlOrTable.toUpperCase().contains( "SELECT" ) )
        ...
        if (action.getIsSqlServerSource ) {
          ...
          s"""( SELECT ${action.getPrimaryKeysList.mkString("[", "], [", "]")}  FROM ${uniqueKeysSqlOrTable} ) AS tbl${java.util.UUID.randomUUID.toString.replace("-", "")} """
        }
        else if (action.getIsOracleSource)

          // The Oracle query will look like this: ( SELECT list-of-unique-keys FROM source-table )
          s"""( SELECT ${action.getPrimaryKeysList.mkString(",")}  FROM ${uniqueKeysSqlOrTable} ) """
        else
       ...
       */
        ...
        s"""[ { $$project : { _id: 0, ${action.getPrimaryKeysList.map(s => s""" "$s": { $$ifNull: [ "$$$s", null ] } """).mkString( "," ) } } } ] """
      }
    }
    else
      throw new RuntimeException("""ERROR: Cannot build Query To Get All Unique Keys - unsupported DBMS""")

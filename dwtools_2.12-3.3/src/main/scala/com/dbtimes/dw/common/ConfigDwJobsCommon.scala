...
private[dw] class ConfigDwJobsCommon {
  ...
  protected def getSchema(schemaJsonRoot: Config, schemaJsonPath: String): StructType = {
    if ( schemaJsonRoot.hasPath(schemaJsonPath) ) {
      ...
      // "colType" is only required if isPartitionKey is true, and that in turn is needed to get correct result of min and max values
      // If column does not have "colType", exclude that column from schema as StructField requires "colType".
      // All columns will have "colName" as it enforced by schema validation
      val columnsWihType = sourceSchema filter {
        case column: Config => {
          if (column.hasPath("colType")) true else false
        }
      }

      // columnsWihType may be empty if none of the columns have "colType". "colType" is only required when isPartitionKey is true
      if ( columnsWihType.isEmpty ) {
        StructType(Seq())
      }
      else {
        val columns = columnsWihType map {
          case column: Config => {
  ...
  protected def getSchemaColNamesAndPropertyValues(
    ...
    if ( schemaJsonRoot.hasPath(schemaJsonPath) ) {
      ...
      val columns = sourceSchema filter {
        case column: Config => {
          // this call will not return true if the value for the property is null.
          // That is what we want for unknown property (because absent property
          // will be treated as if the value is null) and other properties
  ...
  /**
   *
   * @param fieldName           is the name of the field used to filter columns
   * @param valueToInclude     is the value that will result in inclusion of the column in the result
   * @param valueDefault        is the default value to be used if the field does not appear in the column definition.
   *                           If None - there is no default
   */
  protected def getSchemaColNamesWithIntValue(
      schemaJsonRoot: Config,
      schemaJsonPath: String,
      fieldName: String,
      valueToInclude: Int,
      valueDefault: Option[Int]): List[String] = {

    if ( schemaJsonRoot.hasPath(schemaJsonPath) ) {
      val sourceSchema = schemaJsonRoot.getConfigList(schemaJsonPath).asScala.toList
      val columns = sourceSchema filter {
        case column: Config => {
          if (column.hasPath(fieldName)) {
            if (column.getInt(fieldName) == valueToInclude)
              true
            else
              false
          }
          else if (valueDefault.isDefined && valueToInclude == valueDefault.get)
            true
          else
            false
        }
   ...
   */
  def getConfigObjectFieldsAndValues(
    ...
    if (configObjectAsOption.isDefined) {
      val attributes = configObjectAsOption.get
        .entrySet()
        .asScala
        .map(e => e.getKey.stripPrefix("\"").stripSuffix("\"") -> {
          if( e.getValue.unwrapped().getClass.toString == "class java.util.ArrayList" )
            e.getValue.unwrapped().asInstanceOf[java.util.ArrayList[String]].asScala.mkString( "\n" ) // this will break array strings of the query with new line which  will allow for two dash comments inside any of the strings
          else
            e.getValue.unwrapped().toString
        } )
        .toMap

      // val filteredOfComments = attributes.view.filterKeys(key => !key.startsWith( "__" ) ).toMap  // scala 2.13
      val filteredOfComments = attributes.filterKeys(key => !key.startsWith( "__" ) ).toMap // Refactored for Scala 2.12
      filteredOfComments
    }
    else {
      Map[String,String] ()

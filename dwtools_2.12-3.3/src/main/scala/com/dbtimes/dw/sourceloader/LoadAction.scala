...
*/
  ...
  override private[dw] def getIsDebugDwLib: Boolean = if (!action.hasPath("action.isDebugDwLib")) false else action.getBoolean("action.isDebugDwLib") // no debug by default
  ...
  override private[dw] def getPrimaryKeysList: List[String] = super.getSchemaColNamesWithFlag(
    action, "action.schema", "isUniqueKey", true, Some(false))

  override private[dw] def getExcludeFromVersioningColumnList: List[String] = super.getSchemaColNamesWithFlag(
    action, "action.schema", "isExcludeFromVersioning", true, Some(false))

  // Methods applicable to Load Control configuration
  override private[dw] def getIsMaintainLoadControl: Boolean = if (getIsMaintainLoadControlAsParquetFile /* || getIsMaintainLoadControlAsTable */ ) true else false

  override private[dw] def getIsMaintainLoadControlAsParquetFile: Boolean = if (action.hasPath("action.fileLoadControl.parquet")) true else false

  override private[dw] def getLoadControlParquetFileDir: String = if (getIsMaintainLoadControlAsParquetFile) action.getString("action.fileLoadControl.dir") else throw new RuntimeException("""Loader Configuration ERROR: Load Control is not defined as a parquet file. """)
  ...
  override private[dw] def getSourceTypeDescription: String = if (getIsFileSource && getIsFileSourceCsv) "csv file" else if (getIsDbmsSource) "DBMS table" else "Unknown"

  override private[dw] def getSourceDescription: String = {
  ...
  override private[dw] def getIsVersioned: Boolean = if (!action.hasPath("action.fileDestination.isVersioned")) {
  ...
  override private[dw] def getMetadataColumnPrefix: String = if (!action.hasPath("action.fileDestination.metadataColumnsPrefix")) {
    "" // There is no prefix by default
  }
  else {
    action.getString("action.fileDestination.metadataColumnsPrefix")
  }
  ...
  override private[dw] def getIsFileDestinationParquet: Boolean = if (getIsFileDestination && action.hasPath("action.fileDestination.parquet")) true else false

  override private[dw] def getDestinationFilePath: String = if (getIsFileDestination) action.getString("action.fileDestination.path") else throw new RuntimeException("""Loader Configuration ERROR: Destination is not a file """)

  override private[dw] def getIsSavePreviousVersionOfDestinationFile: Boolean = if (!getIsFileDestination || !action.hasPath("action.fileDestination.previousCopyPath")) false else true

  override private[dw] def getFileDestinationSavePreviousVersionAs: String = if (getIsSavePreviousVersionOfDestinationFile) action.getString("action.fileDestination.previousCopyPath") else throw new RuntimeException("""Loader Configuration ERROR: "fileSource.previousCopyPath" is not specified """)
  ...
  private[sourceloader] def getPostProcessPackageName: String = {
    if (getIsPostProcessDefined)
      if (action.hasPath("action.postProcess.packageName"))
        action.getString("action.postProcess.packageName")
      else
        ""
    else
...
/*
  ...
  override private[dw] def getIsInitialLoad: Boolean = {

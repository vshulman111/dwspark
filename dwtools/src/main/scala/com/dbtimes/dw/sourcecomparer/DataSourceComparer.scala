...
object DataSourceComparer {
  ...
  def compare(appConfig: Config ): Unit = {
    ...
    DataSourceComparer.validateConfig(appConfig: Config)
...
*/
  ...
  private def compareSources(scenario: ScenarioConfig, dfLeft: DataFrame, dfRight: DataFrame): DataFrame = {
    ...
    if (!nonUniqueKeys.isEmpty) {
      ...
      val diffsNotEqualList = for (nonUniqueKeyColumn <- nonUniqueKeys;
                                   columnType = dfLeftWithUniqueKey.schema(nonUniqueKeyColumn).dataType.typeName
                                   )
      yield if (List("float", "double").contains(columnType))
        getDiffsForNotEqualFloat(scenario, nonUniqueKeyColumn, leftWithUniqueKeyViewName, rightWithUniqueKeyViewName)
      else if (List("date", "timestamp").contains(columnType))
        getDiffsForNotEqualDateTimestamp(scenario, nonUniqueKeyColumn, leftWithUniqueKeyViewName, rightWithUniqueKeyViewName, columnType)
      else
        getDiffsForNotEqualNonFloatDateTimestamp(scenario, nonUniqueKeyColumn, leftWithUniqueKeyViewName, rightWithUniqueKeyViewName)
  ...
  private def addUniqueColumnToSource(scenario: ScenarioConfig, df: DataFrame): DataFrame = {
    ...
    df.withColumn("UniqueKeyConcat",
        {
          val value = concatColumns(struct(uniqueKeys.head, uniqueKeys.tail: _*)).cast(StringType);
  ...
  private def getDiffsForNotEqualFloat(scenario: ScenarioConfig, columnName: String, viewNameLeft: String, viewNameRight: String): DataFrame = {
    ...
    val sqlDiffsNotEqualFloat =
      s"""
         ...
         |WHERE ABS( IFNULL( leftSrc.`$columnName`, 0 ) - IFNULL( rightSrc.`$columnName`, 0 ) ) > ${scenario.getFloatThreshold.toString}
         |${if (scenario.getMaxSameDifferences.isDefined) "LIMIT " + scenario.getMaxSameDifferences.get else ""}
         |    """.stripMargin
    comparerLog.debug(s"  Creating data frame with differences for floating point column `$columnName` using sql:\n" + sqlDiffsNotEqualFloat)
  ...
  private def getDiffsForNotEqualDateTimestamp(scenario: ScenarioConfig, columnName: String, viewNameLeft: String, viewNameRight: String, columnType: String): DataFrame = {
    val spark = SparkSession.builder().getOrCreate() // this gets previously created session
    val uniqueKeys = scenario.getUniqueKeyColumns
    val sqlDiffsNotEqualDateTimestamp =
      s"""
         |SELECT
         |	${uniqueKeys.mkString("\n  leftSrc.`", "`,\n  leftSrc.`", "`")},
         |	'$columnName'	AS `Column Name`		,
         ...
         |            AND ${
        if (columnType == "timestamp" && scenario.getTimestampTruncateUnit.isDefined)
          "DATE_TRUNC( '" + scenario.getTimestampTruncateUnit.get + "', leftSrc.`" + columnName + "`)"
        else if (columnType == "date" && scenario.getDateTruncateUnit.isDefined)
          "TRUNC( leftSrc.`" + columnName + "`, '" + scenario.getDateTruncateUnit.get + "' )"
        else
          "leftSrc.`" + columnName + "` "
      }
         | != ${
        if (columnType == "timestamp" && scenario.getTimestampTruncateUnit.isDefined)
          ...
          "TRUNC( rightSrc.`" + columnName + "`, '" + scenario.getDateTruncateUnit.get + "' )"
        else
          "rightSrc.`" + columnName + "` "
      }
         |    )
         |${if (scenario.getMaxSameDifferences.isDefined) "LIMIT " + scenario.getMaxSameDifferences.get else ""}
         |            """.stripMargin
    comparerLog.debug(s"  Creating data frame with differences for DateTimestamping point column `$columnName` using sql:\n" + sqlDiffsNotEqualDateTimestamp)
    val dfDiffsNotEqualDateTimestamp = spark.sql(sqlDiffsNotEqualDateTimestamp)
    if (scenario.getIsDebugDwLib) {
      dfDiffsNotEqualDateTimestamp.show(3)
    }
    dfDiffsNotEqualDateTimestamp
  }

  private def getDiffsForNotEqualNonFloatDateTimestamp(scenario: ScenarioConfig, columnName: String, viewNameLeft: String, viewNameRight: String): DataFrame = {
    val spark = SparkSession.builder().getOrCreate() // this gets previously created session
    val uniqueKeys = scenario.getUniqueKeyColumns
    val sqlDiffsNotEqualNonFloat =
      s"""
         ...
         |${if (scenario.getMaxSameDifferences.isDefined) "LIMIT " + scenario.getMaxSameDifferences.get else ""}
         |            """.stripMargin
    comparerLog.debug(s"  Creating data frame with differences for non-floating point column `$columnName` using sql:\n" + sqlDiffsNotEqualNonFloat)
  ...
  private def saveDifferences(scenario: ScenarioConfig, dfDifferences: DataFrame): String = {
    ...
    val resultFile = FileHelper.makePath(scenario.getCompareResultDir,
    ...
    FileHelper.saveDataFrameAsCsv(resultFile, dfDifferences)
  }

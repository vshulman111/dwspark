...
private[sourcecomparer] class LoaderOfOracleSource(private val scenario: ScenarioConfig, private val isLeftSource: Boolean) extends ScenarioSourceLoader {

  private[sourcecomparer] def loadSource( ): DataFrame = {
    val df = DataFrameHelper.readDfFromOracle(
      ...
      StructType(Seq()) )  // do not use schema for oracle because datatype inference is good and because of that the schema is either absent or incomplete in the config file

...
private[sourcecomparer] class ScenarioConfig(val scenarioConfig: ConfigValue) extends ConfigDwJobsCommon {
  ...
  private[sourcecomparer] def getFloatCompareThreshold: Double = {
    if (scenario.hasPath("scenario.floatCompareThreshold") ) {
      scenario.getDouble("scenario.floatCompareThreshold")
    }
    else {
  ...
  private[sourcecomparer] def getSelectClauseToCreateSubsetToCompare( isLeftSource: Boolean): Option[String] = {
    if ( scenario.hasPath(s"scenario.${getLeftOrRight(isLeftSource)}Source.selectClauseWithFieldsToCompare")) {
      val selectClause: ConfigValue = scenario.getValue( s"scenario.${getLeftOrRight(isLeftSource)}Source.selectClauseWithFieldsToCompare" )
      if ( selectClause.unwrapped().getClass.toString == "class java.util.ArrayList" ) {
        Some( selectClause.unwrapped().asInstanceOf[java.util.ArrayList[String]].asScala.mkString )  // Convert array to string
      }
      else // string
        Some(scenario.getString(s"scenario.${getLeftOrRight(isLeftSource)}Source.selectClauseWithFieldsToCompare"))
    } else
      None
  }
  ...
  private[sourcecomparer] def getSourceTable( isLeftSource: Boolean): Option[String] = {
    if (getIsDbmsSource( isLeftSource ) ) {
      ...
      else if ( getIsDbmsSourceMongoDb(isLeftSource) && dbmsAttributes.contains("collection") ) {
        dbmsAttributes.get("collection")
      }
      else {
        None
      }

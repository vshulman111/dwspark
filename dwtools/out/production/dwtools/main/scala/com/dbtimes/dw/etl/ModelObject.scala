...
import com.dbtimes.dw.common.{FileHelper, LogFile, MiscHelper}
...
object ModelObject {
  ...
  def runEtl(appConfig: Config): Unit = {
    ...
    val errors: Seq[String] = ModelObject.validateConfig(appConfig: Config)
    if (errors.nonEmpty) {
      var errorMessage: String = ""
      errors.foreach(error => errorMessage = errorMessage + s"* ${error}\n")
      throw new RuntimeException("DW ETL Configuration ERROR(s)\n" + errorMessage)
    }
  ...
  private def getEffectiveDatesToProcessForIncrLoad(configDwEtl: ConfigDwEtl, modelObjectName: String, dfDatesOverriddenBySpecificObjectAsOption: Option[DataFrame] ): DataFrame = {
    ...
    val dfEffDate = {
      ...
      val sqlEffDateFromStgSource =
        s"""| WITH MaxLastProcessedEffDate
            | AS
            ...
            | MaxEffDateToProcess
            | AS
            | ( SELECT MAX( $effectiveDateColName ) AS $effectiveDateColName
            |    FROM $viewEffectiveDates
            |  )
            ...
            |  ( SELECT LastProcessedEffDate FROM MaxLastProcessedEffDate )
            |UNION -- this part will be used to catch-up sources that that changed on the last effective date when some of the sources, including the one changed, were already run
            |SELECT MAX( effDateSource.$effectiveDateColName ) AS $effectiveDateColName
            |FROM $viewEffectiveDates AS effDateSource
            |WHERE ( SELECT $effectiveDateColName FROM MaxEffDateToProcess ) = ( SELECT LastProcessedEffDate FROM MaxLastProcessedEffDate )
            |  AND EXISTS
            |  ( SELECT 1
  ...
  private[etl] def createEtlLogRecord(
    ...
    val dfEtlRecordList = for (
      ...
      maxTimestampAsOption = ModelObject.getStagingSourceMaxTimestamp(stgSourceMoniker, configDwEtl);
      dfEtlRecord = ModelObject.createEtlRecordDataFrame(modelObjectName, Some(stgSourceMoniker), maxTimestampAsOption, if (datesToProcess.isEmpty) None else Some(datesToProcess.last), isInitialLoad, isRerun, durationSec);
    ...
    val dfEtlRecordsForAllSources = if (!dfEtlRecordList.isEmpty) {
      ...
      ModelObject.createEtlRecordDataFrame(modelObjectName, None, None, if (datesToProcess.isEmpty) None else Some(datesToProcess.last), isInitialLoad, isRerun, durationSec);
    }
...
private[etl] abstract class ModelObject(private val modelObjectName: String) {
  ...
  protected val spark: SparkSession = SparkSession.builder().getOrCreate() // this gets previously created session

  import spark.implicits._

  protected val effDateColumnNameInDatesToProcess: String = ModelObject.effDateColumnNameInDatesToProcess
  protected val isInitialLoad: Boolean = configDwEtl.getIsInitialLoad
  protected val viewWithDatesToProcess: String = "DatesToProcess_" + modelObjectName + "_" + java.util.UUID.randomUUID.toString.replace("-", "_")
  ...
  protected def isStgSourceChangedSinceLastLoad(stgSourceTempView: String): Boolean = isStgSourceChangedSinceLastLoadIfKnown
    .getOrElse(stgSourceTempView, Some(true)) // When a name passed to this function is not the correct source, just return true
    .getOrElse(true)

  protected def getLastProcessedStgSourceTimestamp(stgSourceTempView: String): Option[SqlTimestamp] = lastProcessedStgSourceTimestamp
    .getOrElse(stgSourceTempView, None ) // When a name passed to this function is not the correct source, return None

  protected def etlModelObject(stgSourceTempViews: List[String], datesToProcess: List[Date]): Option[DataFrame] // Both dimensions and facts must override this method
  protected def preProcess(stgSourceTempViews: List[String], datesToProcess: List[Date]): Unit = () //preProcess is called before the etl method starts processing. override to create multiple views on the same source
  protected def postProcess(stgSourceTempViews: List[String], datesToProcess: List[Date], dfModelObject: DataFrame): DataFrame = dfModelObject // postProcess is called right before the df is saved. Overrode if needed

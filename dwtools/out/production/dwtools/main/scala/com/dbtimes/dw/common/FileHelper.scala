...
object FileHelper {
  ...
  private def moveDirectoryFiles(sourceDir: String, destDir: String, filterExpression: String => Boolean = (filePath: String) => true): Unit = {
    ...
    filesInSourceDirectory.foreach { fileName =>
      fs.rename(
        ...
        new HadoopPath(fileName.replaceAllLiterally(sourceDirWithNoLastSeparator, destDirWithNoLastSeparator)))
    }
  }
   ...
   * @param destDir
   */
  def moveDirectory(sourceDir: String, destDir: String): Unit = {
    val spark = SparkSession.builder().getOrCreate() // this gets previously created session
    val fs = FileSystem.get(spark.sparkContext.hadoopConfiguration)
  ...
  def deleteDirectoryOrFileIfExists(dirOrFile: String): Unit = {
  ...
  def saveDataFrameAddToExisting(
      df: DataFrame,
      filePath: String,
      filePathPrevVersion: String = null,
      columnsPartitionBy: List[String] = List.empty): Unit = {
    ...
    if (fs.exists(new HadoopPath(filePath))) {
      ...
      if (!columnsPartitionBy.isEmpty) {
        ...
        val newPartitions = FileHelper.getFilesInDirectory(filePath + uniqueId, true, false, true)
          ...
          .map(_.replaceAllLiterally(filePath + uniqueId, "")) // only take the part that has partitions. It will also have path separator as a first character
          ...
          if (!fs.exists(new HadoopPath(filePath + newPartition))) {
            ...
            filesInExistingPartition.foreach { newFile =>
              fs.rename(
                ...
                new HadoopPath(newFile.replaceAllLiterally(filePath + uniqueId, filePath)))
            }
          }
        }
      }
  ...
  def saveDataFrameAsParquetReplacePartitions(
      df: DataFrame,
      filePath: String,
      filePathPrevVersion: String = null,
  ...
  def saveDataFrameAsParquetAndMoveToParentDir(
      df: DataFrame,
  ...
  def removePathEndingSeparator(dirName: String): String = {
    val fileSeparator = FileSystems.getDefault().getSeparator()
   ...
   */
  def concatCsvFiles(destFilePath: String, srcDirectories: List[String] ): Unit = {
    val spark = SparkSession.builder().getOrCreate() // this gets previously created session
  ...
  def moveFirstCsvFileInDirectory(destFilePath: String, srcDirectory: String ): Unit = {
    val spark = SparkSession.builder().getOrCreate() // this gets previously created session
    ...
    if ( fileList.size > 0 ) {
      val fs = FileSystem.get(spark.sparkContext.hadoopConfiguration)
      fs.rename( new HadoopPath(fileList(0)), new HadoopPath(destFilePath) )

...
abstract private[sourcecomparer] class ScenarioSourceLoader {
  ...
  protected def createSubsetToCompare( scenario: ScenarioConfig, isLeftSource: Boolean, dfBase: DataFrame ): DataFrame = {
    ...
    if( scenario.getSelectClauseToCreateSubsetToCompare( isLeftSource ).isDefined ) {
      ...
      val sqlQueryToCreateSubsetToCompare = scenario.getSelectClauseToCreateSubsetToCompare( isLeftSource ).get + " FROM " + viewNameBaseDf
      comparerLog.info(s"  Creating subset data frame for actual compare of ${scenario.getMoniker(isLeftSource)} data source using sql:\n" + sqlQueryToCreateSubsetToCompare )
